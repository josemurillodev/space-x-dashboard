...
export default function RocketsChart({ data }: { data: {
  ...
  return (
    <div className="bg-black p-8 rounded-xl border border-zinc-900 relative overflow-hidden group">
      ...
      {/* TODO: add stats */}
      <p className="text-zinc-500 text-xs mt-1 uppercase tracking-tighter mb-4">
            ...
            return (
              <div key={name} className="group cursor-default">
                <div className="flex justify-between items-end mb-1 px-1">
                  <span className={`text-xs font-mono uppercase tracking-tighter ${ROCKETS_COLORS[i]?.text ?? 'text-zinc-500'}`}>
                    {name}
                  </span>
